...
- Node.js 20 or newer and npm (or bun)
...
The frontend runs against the live deployed contracts out of the box, so you don't need to deploy anything to develop UI against testnet or mainnet.
...
Mainnet equivalents (`*_MAINNET`) are in [Smart Contracts → Deployed addresses](/architecture/contracts/#deployed-addresses). Optionally set `NEXT_PUBLIC_SUBGRAPH_URL` / `SUBGRAPH_URL` to enable [subgraph-backed](/developers/subgraph/) listing and activity queries. Omit them and the app reads the chain directly.
...
:::caution
The deployment order is not cosmetic. Steps 5 through 8 close real attack surface (the payment-routing restriction and the 48-hour fee timelock). Use the script rather than deploying by hand; see [Security](/architecture/security/) for what each step enforces.
:::
