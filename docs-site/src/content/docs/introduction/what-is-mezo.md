...
**Mezo is a Bitcoin-focused Layer 2 network.** Its goal is to make Bitcoin *productive*: instead of BTC sitting idle in a wallet, Mezo lets you use it in applications — borrowing, earning, governance, and trading — while staying anchored to Bitcoin.

## The essentials

| Property | What it means for you |
|---|---|
| **EVM-compatible** | Mezo runs the Ethereum Virtual Machine, so it works with MetaMask and every standard EVM wallet, and developers deploy Solidity contracts to it — but the economy is denominated in Bitcoin. |
...
Each lock is represented as an **NFT** — this is the key fact for Vezo. Because the position is an NFT (an ERC-721 token), it is *transferable*: selling the NFT sells the entire locked position, including its remaining lock, its voting power, and its claim on future rewards. That transferability is what makes a secondary market possible, and it's explained in depth in [Vote-Escrow & veNFTs](/introduction/vote-escrow/).
...
- [mezo.org](https://mezo.org) — official Mezo site
