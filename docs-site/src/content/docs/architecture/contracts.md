...
- Validates ownership **and** approval at list time, then **re-validates ownership at buy time** — a listing can never be used to sell an NFT the seller no longer holds.
- Enforces the settlement order: state closed → NFT transferred → payment routed (checks-effects-interactions, reentrancy-guarded).
...
A **read-only** adapter around Mezo's vote-escrow contracts (veBTC and veMEZO). Answers, for any token ID: locked amount (intrinsic value), decay-adjusted voting power, lock expiry, expiry status, and collection support. Holds no funds, mutates no state; it isolates every Mezo-specific quirk from the trading core. The deployed vote-escrow contracts are EIP-1967 proxies over a Velodrome-v2-style implementation, whose interface differences (four-field lock struct with `isPermanent`, no `balanceOfNFT`/`tokensOfOwner` helpers) are absorbed here and in the frontend.

### PaymentRouter

Moves money. Splits every settlement into protocol fee → treasury and remainder → seller, supporting native BTC (value transfer) and ERC-20s (MEZO, MUSD) uniformly.
...
| `FEE_MANAGER_ROLE` | Propose/execute fee changes — subject to the 48-hour timelock and 5% hard cap |
| `COLLECTION_MANAGER_ROLE` | Manage supported veNFT collections |

Fee changes are two-phase (`proposeFeeChange` → 48h wait → `executeFeeChange`), refuse to overwrite a pending proposal, and refuse to execute against an unconfigured router.
...
| **SwapPaymentRouter** | Pay-with-any-token purchases | Swaps through the Velodrome-style BTC/MUSD pool (pool-direct, no oracle), then calls `buyNFT` and forwards the NFT — one transaction |
