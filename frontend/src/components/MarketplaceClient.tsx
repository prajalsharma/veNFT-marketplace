...
export default function MarketplaceClient() {
  ...
  const filteredListings = useMemo(() => {
    ...
    const filtered = searchableListings.filter((listing) => {
      ...
      const discountMatch = Number(listing.discountBps) / 100 >= minDiscount;
    ...
    const sorted = [...filtered].sort((a, b) => {
      ...
      if (a.discountBps > b.discountBps) return -1;
      if (a.discountBps < b.discountBps) return 1;
      return 0;
    });
  ...
  const activeCount = useMemo(
    () => searchableListings.filter((listing) => listing.active).length,
    [searchableListings]
  );
