...
// ─── Adapter ABI (minimal — only what BuyModal needs) ─────────────────────────
...
export function BuyModal({ isOpen, onClose, listing, onPurchaseSuccess }: BuyModalProps) {
  ...
  // ── Expired veNFT check — query adapter.isExpired before allowing purchase ──
  ...
  // Treat undefined (still loading) as not-expired so we don't block prematurely
  ...
  useEffect(() => {
    if (txConfirmed && step === "buying") {
      // Refresh marketplace state so the listing count and user listings update
      refetchCount();
      refetchUserListings();
      // Notify parent to refresh veNFT balances (portfolio "In Your Wallet" sections)
      onPurchaseSuccess?.();
      setStep("done");
    }
  ...
  async function handleBuy() {
    ...
    if (isVeNFTExpired) {
      setErrorMsg(
        "This veNFT's lock has expired and cannot be traded. The listing should be cancelled by the seller."
      );
      setStep("error");
      return;
    }
      ...
      {isOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <motion.div
            ...
            className="relative w-full max-w-md bg-[#0a0a0a] border border-white/[0.08] rounded-[2.5rem] overflow-hidden shadow-2xl"
          >
            {/* Header */}
            <div className="p-8 border-b border-white/[0.06] flex justify-between items-center">
              <div>
                <h2 className="text-2xl font-bold">
                  ...
                  <span className="text-[#F7931A]">#{listing.tokenId.toString()}</span>
                </h2>
            ...
            <div className="p-8 space-y-5">
              ...
              {isVeNFTExpired && step === "confirm" && (
                <div className="flex gap-3 p-4 rounded-2xl bg-red-500/10 border border-red-500/20">
                  ...
                  <div>
                    <p className="text-sm font-bold text-red-400">Lock expired</p>
                    <p className="text-xs text-mezo-muted mt-0.5">
                      This veNFT's lock period has ended. Expired positions cannot be traded — the seller needs to cancel this listing.
                    </p>
                  </div>
                </div>
              )}
              ...
              {/* Security note */}
              <div className="flex gap-3 p-4 rounded-2xl bg-[#F7931A]/[0.05] border border-[#F7931A]/20">
              ...
              {step === "done" && (
                ...
                >
                  ...
                  <div>
                    <p className="text-sm font-bold text-emerald-400">Purchase complete!</p>
