...
export function ListingModal({ isOpen, onClose, veNFT }: ListingModalProps) {
  ...
  const formattedIntrinsic = parseFloat(formatEther(veNFT.intrinsicValue)).toFixed(5);
      ...
      {isOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <motion.div
            ...
            className="relative w-full max-w-lg bg-[#0a0a0a] border border-white/[0.08] rounded-[2.5rem] overflow-hidden shadow-2xl"
          >
            {/* Header */}
            <div className="p-8 border-b border-white/[0.06] flex justify-between items-center">
              <div>
                <h2 className="text-2xl font-bold">List {veNFT.collection} <span className="text-[#F7931A]">#{veNFT.tokenId.toString()}</span></h2>
                <p className="text-white/40 text-sm mt-1">Configure your secondary market listing.</p>
              </div>
            ...
            <div className="p-8 space-y-8">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="p-4 rounded-2xl bg-white/[0.03] border border-white/[0.06]">
                  <p className="stat-label mb-1.5">Intrinsic Value</p>
                  <p className="text-lg font-bold tabular-nums leading-none">{formattedIntrinsic} {veNFT.collection === 'veBTC' ? 'BTC' : 'MEZO'}</p>
                </div>
                <div className="p-4 rounded-2xl bg-white/[0.03] border border-white/[0.06]">
                  <p className="stat-label mb-1.5">Voting Power</p>
                  <p className="text-lg font-bold tabular-nums leading-none">{parseFloat(formatEther(veNFT.votingPower)).toFixed(2)}</p>
                </div>
              </div>
              ...
              <div className="space-y-4">
                ...
                <div className="relative group">
                  <input
                    ...
                    placeholder="0.00"
                    className="w-full bg-white/[0.03] border border-white/[0.08] rounded-2xl p-4 text-2xl font-bold focus:outline-none focus:border-[#F7931A]/50 focus:bg-white/[0.04] transition-all pl-6 pr-32"
                  />
                      ...
                      <button
                        ...
                        className={`px-3 rounded-xl text-xs font-black transition-all ${
                          paymentToken === token.symbol
                            ? "bg-[#F7931A] text-black shadow-lg"
              ...
              {/* Safety Info */}
              <div className="p-4 rounded-2xl bg-[#F7931A]/[0.05] border border-[#F7931A]/20 flex gap-4">
                <div className="p-2 bg-[#F7931A]/10 rounded-lg h-fit">
